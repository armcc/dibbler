...
int dns_add(const char * ifname, int ifaceid, const char * addrPlain) {
    ...
    if (!f && !(f=fopen(RESOLVCONF_FILE, "a+")) ) {
        sprintf(error_message(), "Unable to open %s file:%s",
                RESOLVCONF_FILE, strerror(errno));
                  ...
                  unsigned long prefered, unsigned long valid)
{
    ...
    */
    ...
    if (result == 0)
        ...
        sprintf(error_message(), "Unable to update prefix: %s", strerror(errno) );
        return LOWLEVEL_ERROR_UNSPEC;
    }
}
...
int prefix_del(const char* ifname, int ifindex, const char* prefixPlain, int prefixLength) {
    ...
    if (result == 0)
        ...
        sprintf(error_message(), "Unable to delete prefix: %s", strerror(errno));
        return LOWLEVEL_ERROR_UNSPEC;
    }
}
